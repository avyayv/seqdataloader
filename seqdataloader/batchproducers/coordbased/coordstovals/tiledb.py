...
class BasicTileDbSparseArray(CoordsToVals):
    def __init__(self, tiledb_paths, pos_label_source_attribute, neg_label_source_attribute=None, center_size_to_use=None, **kwargs):
        '''
        tiledb_paths can be a single string or a list of strings or a dictionary mapping from  mode name to string. 
        '''
    ...
    def query_tiledb(self,cur_tiledb_path,coords):
        ...
        for i in range(len(coords)):
            coord=coords[i]
            n_data_dnafrag=dnafrag.load(cur_tiledb_path)
            n_data_dnafrag_chr = n_data_dnafrag[coord.chr]
            dnafrag_array=np.zeros((1000,2010))
            n_data_dnafrag_chr.add_to_array(coord.start+1005,dnafrag_array)
            dnafrag_array=np.transpose(dnafrag_array)
            if coord.isplusstrad == False:
                dnafrag_array = dnafrag_array[::-1]
            labels[i] = dnafrag_array
